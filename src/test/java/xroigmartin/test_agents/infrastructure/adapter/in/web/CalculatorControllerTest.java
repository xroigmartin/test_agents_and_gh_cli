...
class CalculatorControllerTest {
  ...
  void should_return_operation_result_when_use_case_succeeds() throws Exception {
    // Given
    when(useCase.calculate(any()))
        .thenReturn(new CalculationResult(Operation.ADDITION, new BigDecimal("8")));
    ...
    ArgumentCaptor<CalculateOperationCommand> captor =
        ArgumentCaptor.forClass(CalculateOperationCommand.class);
    ...
    mockMvc
        .perform(
            post("/api/v1/calculator")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"operation\":\"EXPONENTIATION\",\"firstOperand\":2,\"secondOperand\":3}"))
        .andExpect(status().isBadRequest())
  ...
  void should_return_bad_request_when_use_case_reports_illegal_argument() throws Exception {
    // Given
    when(useCase.calculate(any()))
        .thenThrow(new IllegalArgumentException("Exponent must be an integer value"));
