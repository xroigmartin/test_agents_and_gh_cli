...
/**
 * Default implementation of {@link CalculateOperationUseCase} that delegates the actual mathematics
 * to a domain {@link Calculator}.
 */
public final class DefaultCalculateOperationUseCase implements CalculateOperationUseCase {
  ...
  public CalculationResult calculate(CalculateOperationCommand command) {
    ...
    BigDecimal result =
        resolveCalculation(command.operation(), command.firstOperand(), command.secondOperand());
    return new CalculationResult(command.operation(), result);
  }

  private BigDecimal resolveCalculation(
