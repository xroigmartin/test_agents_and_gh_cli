...
public interface Calculator {
   ...
   * @throws xroigmartin.test_agents.domain.exception.DivisionByZeroException if {@code divisor} is
   *     zero
   */
   ...
   * @throws xroigmartin.test_agents.domain.exception.DivisionByZeroException when computing with a
   *     zero base and a negative exponent
   */
  BigDecimal power(BigDecimal base, BigDecimal exponent);

  /**
   * Calculates the percentage represented by {@code percent} over {@code base}. {@code percent} is
   * treated as a value between -infinity and +infinity (e.g. 15 means 15%).
   *
